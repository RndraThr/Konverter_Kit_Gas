...
export function SettingsPage() {
  ...
  return <div className="page"><header className="pageHeader"><div><h1>Pengaturan sistem</h1><p>Nilai dasar yang digunakan secara konsisten di seluruh program.</p></div></header>
    {query.isError ? <div className="errorState">Pengaturan belum dapat dimuat.</div> : <form onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate(); }} className="dialogBody" style={{ maxWidth: 760, padding: 0 }}>
      {query.data?.data.map((setting) => options[setting.key] ? <label className="formField" key={setting.key}><span>{labels[setting.key]}</span><select className="selectField" disabled={!canManage} aria-label={labels[setting.key]} value={values[setting.key] ?? ''} onChange={(e) => setValues({ ...values, [setting.key]: e.target.value })}>{options[setting.key].map((item) => <option key={item}>{item}</option>)}</select><small>{setting.description}</small></label> : <FormField key={setting.key} label={labels[setting.key] ?? setting.key} name={setting.key} disabled={!canManage} maxLength={setting.key === 'organization_name' ? 160 : 120} value={values[setting.key] ?? ''} onChange={(e) => setValues({ ...values, [setting.key]: e.target.value })} hint={setting.description} />)}
      ...
      {canManage && <div className="fullField"><button className="primaryButton" disabled={mutation.isPending}>Simpan pengaturan</button></div>}
    </form>}
  </div>;
}
