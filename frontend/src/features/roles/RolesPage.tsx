...
export function RolesPage() {
  ...
  return <div className="page"><header className="pageHeader"><div><h1>Role & akses</h1><p>Susun hak akses berdasarkan tanggung jawab pengguna.</p></div>{canManage && <button className="primaryButton" onClick={() => { setSelected(undefined); setDialogOpen(true); }}><Plus />Tambah role</button>}</header>
    {roles.isError ? <div className="errorState">Role belum dapat dimuat.</div> : <DataTable label="Daftar role"><thead><tr><th>Role</th><th>Hak akses</th><th>Pengguna</th><th>Jenis</th>{canManage && <th>Aksi</th>}</tr></thead><tbody>{roles.data?.data.map((role) => <tr key={role.id}><td><strong>{role.name}</strong><br /><small>{role.code}</small></td><td>{role.permissions.length} permission</td><td>{role.user_count}</td><td>{role.is_system ? 'Role sistem' : 'Role kustom'}</td>{canManage && <td style={{ display: 'flex', gap: 7, alignItems: 'center' }}><button className="iconButton" aria-label={`Edit ${role.name}`} disabled={role.is_system} onClick={() => { setSelected(role); setDialogOpen(true); }}><Pencil /></button><button className="iconButton" aria-label={`Hapus ${role.name}`} disabled={role.is_system || role.user_count > 0} onClick={() => window.confirm(`Hapus role ${role.name}?`) && remove.mutate(role.id)}><Trash2 /></button></td>}</tr>)}</tbody></DataTable>}
    {remove.isError && <p className="formNotice">{(remove.error as ApiError).message}</p>}
    {canManage && <RoleDialog error={save.isError ? (save.error as ApiError).message : undefined} open={dialogOpen} onOpenChange={setDialogOpen} groups={permissions.data?.data ?? []} role={selected} pending={save.isPending} onSave={(values) => save.mutate(values)} />}
  </div>;
}
