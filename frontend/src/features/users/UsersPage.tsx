...
export function UsersPage() {
  ...
  const [pendingDeactivation, setPendingDeactivation] = useState<UserValues>();
  ...
  return <div className="space-y-5">
    ...
    {canManage && <UserDialog error={saveMessage} fields={saveFields} open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) save.reset(); }} roles={roles.data?.data ?? []} user={selected} pending={save.isPending} onSave={(values) => { if (selected?.is_active && !values.is_active) { setPendingDeactivation(values); return; } save.mutate(values); }} />}
    ...
    <AlertDialog open={Boolean(pendingDeactivation)} onOpenChange={(open) => { if (!open) setPendingDeactivation(undefined); }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Nonaktifkan {selected?.full_name}?</AlertDialogTitle><AlertDialogDescription>Pengguna ini tidak dapat masuk sampai diaktifkan kembali.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Batal</AlertDialogCancel><AlertDialogAction onClick={() => { if (pendingDeactivation) save.mutate(pendingDeactivation); setPendingDeactivation(undefined); }}>Nonaktifkan pengguna</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </div>;
}
