...
export function UsersPage() {
  ...
  return <div className="page">
    ...
    {canManage && <UserDialog error={save.isError ? (save.error as ApiError).message : undefined} open={dialogOpen} onOpenChange={setDialogOpen} roles={roles.data?.data ?? []} user={selected} pending={save.isPending} onSave={(values) => { if (selected?.is_active && !values.is_active && !window.confirm(`Nonaktifkan ${selected.full_name}?`)) return; save.mutate(values); }} />}
    {canManage && <UserPasswordDialog error={resetPassword.isError ? (resetPassword.error as ApiError).message : undefined} open={passwordOpen} onOpenChange={setPasswordOpen} user={selected} pending={resetPassword.isPending} onSave={(password) => resetPassword.mutate(password)} />}
  </div>;
}
