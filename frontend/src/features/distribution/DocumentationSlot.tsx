...
export function DocumentationSlot({ slot, onChanged }: { slot: SlotSummary; onChanged: (slot: SlotSummary) => void }) {
  ...
  return <article className={`${styles.documentationSlot} ${complete ? styles.documentationComplete : styles.documentationMissing}`} aria-label={slot.label}>
    <header><div><div className={styles.slotTitle}><h4>{slot.label}</h4><Badge variant={slot.required ? 'default' : 'outline'}>{slot.required ? 'Wajib' : 'Opsional'}</Badge></div><p>Ambil foto yang jelas untuk melengkapi bukti penyerahan.</p><span>{files.length} dari {required} foto wajib</span></div><strong>{complete ? <><CheckCircle2 aria-hidden="true" />Lengkap</> : <><CircleAlert aria-hidden="true" />Belum lengkap</>}</strong></header>
    <div className={styles.mediaGrid}>
      {files.map((file) => <figure key={file.id}><img src={file.content_url} alt={file.original_filename} /><figcaption><span>{file.original_filename}</span>{canManage && <button className={styles.removeMedia} type="button" aria-label={`Hapus ${file.original_filename}`} title="Hapus foto" onClick={() => remove.mutate(file.id)}><Trash2 /></button>}</figcaption></figure>)}
      {pending && <figure className={styles.pendingMedia}><img src={pending.previewURL} alt={`Preview ${slot.label}`} /><figcaption><span>{upload.isError ? 'Upload gagal' : 'Mengunggah...'}</span>{upload.isError && <button className={styles.removeMedia} type="button" aria-label="Coba unggah lagi" title="Coba unggah lagi" onClick={() => upload.mutate(pending)}><RefreshCw /></button>}</figcaption>{upload.isPending && <UploadCloud className={styles.uploadingIcon} aria-hidden="true" />}</figure>}
    </div>
    {canManage && files.length < (slot.max_files ?? 1) && <div className={styles.captureActions}>
      {(slot.input_source ?? 'both') !== 'gallery' && <label className={`secondaryButton ${styles.captureControl}`}><Camera />Buka kamera<input aria-label="Buka kamera" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={(event) => choose(event.target.files?.[0], 'camera')} /></label>}
      {(slot.input_source ?? 'both') !== 'camera' && <label className={`secondaryButton ${styles.captureControl}`}><ImagePlus />Pilih galeri<input aria-label="Pilih galeri" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => choose(event.target.files?.[0], 'gallery')} /></label>}
    </div>}
  </article>;
}
