...
export function SchedulesPanel() {
  ...
  // Nilai tersimpan bisa merujuk ke kabupaten/template yang sudah nonaktif atau tidak published lagi;
  // tanpa opsi ini, Select menampilkan UUID mentah karena tidak menemukan item yang cocok.
  const regencyOrphan = editing && !activeRegencies.some((item) => item.id === values.regency_id)
    ? { id: values.regency_id, label: `${editing.regency?.document_code ? `${editing.regency.document_code} - ` : ''}${editing.regency?.name ?? editing.regency_id} (nonaktif)` }
    : undefined;
  const packageOrphan = editing && !compatiblePackages.some((item) => item.id === values.package_template_version_id)
    ? { id: values.package_template_version_id, label: `${packageName(editing)} (tidak published)` }
    : undefined;
  const documentOrphan = editing && !compatibleDocuments.some((item) => item.id === values.documentation_template_version_id)
    ? { id: values.documentation_template_version_id, label: `${documentName(editing)} (tidak published)` }
    : undefined;
  const allItems = schedules.data?.data ?? [];
  const normalizedSearch = search.trim().toLocaleLowerCase('id-ID');
  const items = allItems.filter((item) => (!normalizedSearch || `${item.name} ${item.program?.name ?? ''} ${item.regency?.name ?? ''} ${item.regency?.document_code ?? ''} ${packageName(item)} ${documentName(item)}`.toLocaleLowerCase('id-ID').includes(normalizedSearch)) && (status === 'all' || item.status === status));
  return <section className="setupPanel" aria-labelledby="schedules-heading"><header className="panelHeading"><div><h2 id="schedules-heading">Jadwal kabupaten</h2><p>Satu jadwal mengunci program, lokasi, dan versi template yang digunakan.</p></div>{canManage && <Button type="button" onClick={() => show()}><Plus />Tambah jadwal</Button>}</header>
    {schedules.isPending ? <DataState kind="loading" title="Memuat data jadwal" description="Mengambil konfigurasi jadwal dan template terbaru." /> : schedules.isError ? <DataState kind="error" title="Data jadwal belum dapat dimuat" description="Periksa koneksi, lalu coba kembali." action={{ label: 'Coba lagi', onClick: () => schedules.refetch() }} /> : <><SetupToolbar entity="jadwal" search={search} onSearchChange={setSearch} status={status} onStatusChange={setStatus} options={[{ value: 'all', label: 'Semua status' }, { value: 'active', label: 'Aktif' }, { value: 'draft', label: 'Draf' }, { value: 'completed', label: 'Selesai' }, { value: 'cancelled', label: 'Dibatalkan' }]} shown={items.length} total={allItems.length} />{items.length ? <><p className="tableScrollHint">Geser tabel untuk melihat seluruh kolom.</p><DataTable label="Daftar jadwal" minimumWidth={1024}><thead><tr><th>Jadwal</th><th>Program</th><th>Kabupaten</th><th>Periode</th><th>Template paket</th><th>Foto</th><th>Status</th>{canManage && <th className="actionColumn">Aksi</th>}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><span className="entityName"><CalendarDays />{item.name}</span></td><td>{item.program?.name ?? item.program_id}</td><td><strong>{item.regency?.document_code}</strong> {item.regency?.name ?? item.regency_id}</td><td>{formatDate(item.start_date)}<small className="subline">s.d. {formatDate(item.end_date)}</small></td><td>{packageName(item)}</td><td>{documentName(item)}</td><td><StatusBadge active={item.status === 'active'} activeText="Aktif" inactiveText={item.status === 'draft' ? 'Draf' : item.status === 'completed' ? 'Selesai' : 'Dibatalkan'} /></td>{canManage && <td className="actionColumn"><Button type="button" variant="ghost" size="icon" aria-label={`Edit ${item.name}`} title={`Edit ${item.name}`} onClick={() => show(item)}><Edit3 /></Button></td>}</tr>)}</tbody></DataTable></> : <DataState kind="empty" title={allItems.length ? 'Tidak ada jadwal yang sesuai' : 'Belum ada jadwal kabupaten'} description={allItems.length ? 'Ubah kata kunci atau filter untuk menampilkan data lain.' : 'Tambahkan jadwal setelah program dan template tersedia.'} />}</>}
    <SetupDialog layout="wide" open={open} onOpenChange={setOpen} title={editing ? `Edit ${editing.name}` : 'Tambah jadwal'} description="Pilih template terbit agar data lapangan konsisten." pending={mutation.isPending} dirty={JSON.stringify(values) !== JSON.stringify(initialValues)} onSubmit={() => mutation.mutate()}>
      <SetupFormSection title="Identitas jadwal" description="Nama yang mudah dikenali pada proses distribusi.">
      ...
      <SetupFormSection title="Program dan wilayah" description="Tentukan program bantuan serta kabupaten pelaksana.">
      <div className="grid min-w-0 gap-2"><Label id="schedule-program-label">Program</Label><Select required value={values.program_id} onValueChange={(value) => setValues({ ...values, program_id: value ?? '', package_template_version_id: '', documentation_template_version_id: '' })}><SelectTrigger className="w-full" aria-labelledby="schedule-program-label"><SelectValue placeholder="Pilih program" /></SelectTrigger><SelectContent>{programs.data?.data.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></div>
      <div className="grid min-w-0 gap-2"><Label id="schedule-regency-label">Kabupaten</Label><Select required value={values.regency_id} onValueChange={(value) => setValues({ ...values, regency_id: value ?? '' })}><SelectTrigger className="w-full" aria-labelledby="schedule-regency-label"><SelectValue placeholder="Pilih kabupaten" /></SelectTrigger><SelectContent>{regencyOrphan && <SelectItem value={regencyOrphan.id}>{regencyOrphan.label}</SelectItem>}{activeRegencies.map((item) => <SelectItem key={item.id} value={item.id}>{item.document_code} - {item.name}</SelectItem>)}</SelectContent></Select></div>
      </SetupFormSection>
      <SetupFormSection title="Template distribusi" description="Template disaring otomatis sesuai jenis program.">
      <div className="grid min-w-0 gap-2"><Label id="schedule-package-label">Template paket</Label><Select required value={values.package_template_version_id} onValueChange={(value) => setValues({ ...values, package_template_version_id: value ?? '' })}><SelectTrigger className="w-full" aria-labelledby="schedule-package-label"><SelectValue placeholder="Pilih template" /></SelectTrigger><SelectContent>{packageOrphan && <SelectItem value={packageOrphan.id}>{packageOrphan.label}</SelectItem>}{compatiblePackages.map((item) => <SelectItem key={item.id} value={item.id}>{item.name} v{item.version}</SelectItem>)}</SelectContent></Select></div>
      <div className="grid min-w-0 gap-2"><Label id="schedule-document-label">Template dokumentasi</Label><Select required value={values.documentation_template_version_id} onValueChange={(value) => setValues({ ...values, documentation_template_version_id: value ?? '' })}><SelectTrigger className="w-full" aria-labelledby="schedule-document-label"><SelectValue placeholder="Pilih template" /></SelectTrigger><SelectContent>{documentOrphan && <SelectItem value={documentOrphan.id}>{documentOrphan.label}</SelectItem>}{compatibleDocuments.map((item) => <SelectItem key={item.id} value={item.id}>{item.name} v{item.version}</SelectItem>)}</SelectContent></Select></div>
      </SetupFormSection>
      <SetupFormSection title="Periode dan pengaturan" description="Atur rentang distribusi, status, serta identitas pelaksana.">
      <div className="grid min-w-0 gap-2"><Label id="schedule-status-label">Status</Label><Select value={values.status} onValueChange={(value) => setValues({ ...values, status: value ?? 'draft' })}><SelectTrigger className="w-full" aria-labelledby="schedule-status-label"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="draft">Draf</SelectItem><SelectItem value="active">Aktif</SelectItem><SelectItem value="completed">Selesai</SelectItem><SelectItem value="cancelled">Dibatalkan</SelectItem></SelectContent></Select></div>
